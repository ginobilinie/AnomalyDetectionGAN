...
from ganComponents import Generator3D, Generator3D_Z, Discriminator3D
...
parser.add_argument("--gpuID", type=int, default=7, help="how to normalize the data")
...
parser.add_argument("--lr_G", type=float, default=5e-3, help="Learning Rate. Default=1e-4")
parser.add_argument("--lr_D", type=float, default=5e-3, help="Learning Rate. Default=1e-4")
...
def main():
    ...
    netG = Generator3D(opt.finalSize*opt.finalSize*opt.finalSize, opt.ngf, gpu_ids=opt.gpuID)
#     netG = define_G(opt.fineSize*opt.fineSize, opt.ngf,opt.init_type, opt.gpuID)
    ...
    if opt.isAdLoss:
        netD = Discriminator3D(opt.ndf, opt.gpuID)
    ...
    path_patients_h5 = '/shenlab/lab_stor5/dongnie/iSeg/h5data/'
    path_patients_h5_test ='/shenlab/lab_stor5/dongnie/iSeg/h5data/'
  

    data_generator = Generator_3D_patches_oneKey(path_patients_h5, opt.batchSize, inputKey='dataT1')
    data_generator_test = Generator_3D_patches_oneKey(path_patients_h5_test, opt.batchSize, inputKey='dataT1')
    if opt.resume:
...
#         residual_source = residual_source.cuda()
        ...
        if opt.isAdLoss:
            ...
            outputG = netG(source) #batchxCHx64x64x64->1
            #print 'outputG.shape: ',outputG.shape
            if len(outputG.size())==4:
            ...
            # print 'outputD_real.shape: ',outputD_real.size(), 'real_label.shape: ', real_label.shape
        ...
        if opt.isAdLoss:
            ...
            #outputG = net(inputs)
            outputG = netG(source) #batch x 128-> batchxCHx64x64x64
            if len(outputG.size())==4:
                outputG = outputG.unsqueeze(1)
...
#             residual_source = residual_source.cuda()
            ...
            if len(outputG.size())==4:
                outputG = outputG.unsqueeze(1)
            outputD_fake = netD(outputG)
            print 'outputD_real is ',outputD_real.data[0], ' outputD_real is ',outputD_real.data[0]


        if iter % opt.showTestPerformanceEvery == 0:  # test one subject
            mr_test_itk=sitk.ReadImage(os.path.join(path_test,'1_QFZ_60s_suv.nii.gz'))
